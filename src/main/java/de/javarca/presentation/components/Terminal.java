package de.javarca.presentation.components;
...
import static de.javarca.presentation.data.JavarcadeProject.APP_MODULES_FOLDER;
import static de.javarca.presentation.data.JavarcadeProject.APP_NO_MODULES_FOLDER;
...
import static de.javarca.presentation.data.JavarcadeProject.EXTRA_INSTALL_FOLDER;
import static de.javarca.presentation.data.JavarcadeProject.WORK_FOLDER;
import static de.javarca.presentation.ui.UI.applyScrollPaneStyleMono;
...
public record Terminal(TextFlow theTerminal, ImageView nuke, StackPane slideView, ScrollPane container) {
    ...
    public void reset(boolean moduleSystem, ShellCommand.Tool focusedTool, boolean rogue, List<Process> activeProcess) {
    ...
    public void resetCurrent(List<Process> activeProcess) {
        destroyProcess(activeProcess);
    ...
    public void execute(boolean moduleSystem, ShellCommand.Tool focusedTool, Set<Module> activeModules,
                        List<Process> activeProcess, ApplicationScreen applicationScreen, Consumer<Path> updateCommand) {
        destroyProcess(activeProcess);
        ...
        new Thread(() -> {
            var succeeded = runExternalCommand(cmd.get(), activeModules, commandText.getText(), activeProcess, applicationScreen);
            if (succeeded) {
                Platform.runLater(() -> {
                    updateCommand.accept(cmd.get().workDir());
                    runExternalCommand(cmd.get().followUp(), activeModules, null, activeProcess, applicationScreen);
                });
            }
    ...
    private void destroyProcess(List<Process> activeProcess) {
        if (!activeProcess.isEmpty()) {
            activeProcess.forEach(Process::destroy);
            activeProcess.clear();
        }
    }

    private boolean runExternalCommand(ShellCommand cmd, Set<Module> activeModules, String override, List<Process> activeProcess, ApplicationScreen applicationScreen) {
        if (cmd == null) {
            return false;
        }
    ...
    private void cleanCaches() {
        try {
            Files.createDirectories(WORK_FOLDER);
